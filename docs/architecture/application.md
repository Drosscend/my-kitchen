...
The application is a pragmatic modular monolith. It uses business capability slices, separates framework-facing delivery from application code, and separates command models from read models.
...
POST /signup
  ...
  -> RegisterUser.execute + SendEmailVerification.execute
  -> UserRepository.createUser, EmailVerificationTokenRepository.replaceForUser
  -> User
...
The account page is a composite page: one render, one execute per form.

POST   /account/profile  -> UpdateProfileController.execute      -> UpdateProfile.execute
POST   /account/email    -> RequestEmailChangeController.execute -> RequestEmailChange.execute
POST   /account/password -> ChangePasswordController.execute     -> ChangePassword.execute
DELETE /account          -> DeleteAccountController.execute      -> DeleteAccount.execute
```
